...
import qt3utils
from qt3utils.applications.qdlscan.application_controller import ScanController
from qt3utils.applications.qdlscan.application_gui import (
    LauncherApplicationView,
    LineScanApplicationView,
    ImageScanApplicationView
)
import qt3utils.applications.qdlscope.main as qdlscope

logger = logging.getLogger(__name__)
logging.basicConfig()


CONFIG_PATH = 'qt3utils.applications.qdlscan.config_files'
DEFAULT_CONFIG_FILE = 'qdlscan_snspd.yaml'
...
class LineScanApplication:
    ...
    def save_scan(self, tkinter_event=None) -> None:
        ...
        with h5py.File(file_path+file_name+'.hdf5', 'w') as df:
            ...
            ds.attrs['application'] = 'qt3utils.qdlscan.LineScanApplication'
        ...
        with h5py.File(file_path+file_name+'.hdf5', 'w') as df:
            ...
            ds = df.create_dataset('file_metadata', 
                                   data=np.array(['application', 
                                                  ...
                                                  'timestamp', 
                                                  'original_name'], dtype='S'))
            ds.attrs['application'] = 'qt3utils.qdlscan.ImageScanApplication'
            ds.attrs['qt3utils_version'] = qt3utils.__version__
            ds.attrs['scan_id'] = self.id
            ds.attrs['timestamp'] = self.timestamp.strftime("%Y-%m-%d %H:%M:%S")
            ds.attrs['original_name'] = file_name
